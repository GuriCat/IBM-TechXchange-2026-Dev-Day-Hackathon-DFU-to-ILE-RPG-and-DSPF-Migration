      ...
      // Reproduces DFU GURILIB/TESTDFU row-based (line mode) behavior
      // Sub-Task 2+3 of dfu-to-rpgle-plan.md
      // ============================================================
      ...
      // File Definitions
      //   QCUSTCDT : update/add capable, QIWS must be on library list
      //   TDSTDSPF : workstation combined file, indicator DS
      //   TDSTPRTR : printer file for audit report
      ...
      // Database I/O Feedback INFDS
      Ddbfds            DS
      D  dbfStatus             11     15
      D  dbfOpr               *OPCODE

      // Workstation indicator DS matching INDARA in TDSTDSPF
      Dws               DS
      ...
      // Session counters -- shared with ENDR format fields
      DADDCNT           S              5P 0 INZ(0)
      DCHGCNT           S              5P 0 INZ(0)
      DDLTCNT           S              5P 0 INZ(0)

      // Internal mode flag: C=Change  I=Input(append)  A=Insert
      DwkMode           S              1A   INZ('C')

      // Work field for DSPLY (52-char max)
      DdspMsg           S             50A
      Dwait             S              1A

      // Return-point variable for #DBEX INFSR
      DreturnPt         S              6A
